...
namespace Lesson_8_ExternalSort
{
    class Program
    {
        private static void SplitBigFile(string file)
        {
            ...
            using (StreamReader reader = new StreamReader(file))
            {
                while (reader.Peek() >= 0)
                {
                    ...
                    //Если чтение файл превышает максимально возможное значение, то создаем новый tmp файл
        ...
        {
            //Считываем разделенные файлы из директории
            foreach (string path in Directory.GetFiles(tempFiles, "split*.txt"))
            {
                string[] content = File.ReadAllLines(path);

                //
                //Временные массив, для того, чтобы произвести сортировку MergeSort(принимает только интовые значения)
                ...
                //Использовал для сортировки этот алгоритм (можно использовать любой)
